...
// shipping adjustment
    ...
    function install() {
      global $db;
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Разрешить модуль Тарифы для зоны', 'MODULE_SHIPPING_ZONES_STATUS', 'True', 'Вы хотите разрешить модуль доставки Тарифы для зоны?', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Метод расчёта', 'MODULE_SHIPPING_ZONES_METHOD', 'Weight', 'Стоимость доставки рассчитывается на основании:<br />weight - общего веса заказа,<br />price - общей стоимости заказа,<br />item - общего количества товаров', '6', '0', 'zen_cfg_select_option(array(\'Weight\', \'Price\', \'Item\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Налог', 'MODULE_SHIPPING_ZONES_TAX_CLASS', '0', 'Использовать следующий класс налогов', '6', '0', 'zen_get_tax_class_title', 'zen_cfg_pull_down_tax_classes(', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Базис налога', 'MODULE_SHIPPING_ZONES_TAX_BASIS', 'Shipping', 'Базис для расчёта налога. Возможные варианты:<br />Shipping - Налог рассчитывается по адресу доставки<br />Billing - Налог рассчитывается по адресу плательщика<br />Store - Налог рассчитывается по адресу магазина, если зона доставки/плательщика совпадает с зоной магазина', '6', '0', 'zen_cfg_select_option(array(\'Shipping\', \'Billing\', \'Store\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Порядок сортировки', 'MODULE_SHIPPING_ZONES_SORT_ORDER', '0', 'Порядок сортировки', '6', '0', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Пропускаемые страны, список двубуквенных ISO-2 кодов стран через запятую', 'MODULE_SHIPPING_ZONES_SKIPPED', '', 'Запретить для следующих стран:', '6', '0', 'zen_cfg_textarea(', now())");
      ...
      for ($i = 1; $i <= $this->num_zones; $i++) {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Страны зоны " . $i ."', 'MODULE_SHIPPING_ZONES_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Список кодов ISO-2 стран для зоны " . $i . ".<br />Установите 00 чтобы включить все страны.', '6', '0', 'zen_cfg_textarea(', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Стоимость доставки для зоны " . $i ."', 'MODULE_SHIPPING_ZONES_COST_" . $i ."', '3:8.50,7:10.50,99:20.00', 'Стоимость доставки для зоны " . $i . " на базе максимальной стоимости/веса заказа. Например: 3:8.50,7:10.50,... Это значит, что стоимость доставки для заказов, весом/суммой до 3 кг. будет стоить 8.50 для покупателей из стран зоны " . $i . ".', '6', '0', 'zen_cfg_textarea(', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Накладные расходы зоны " . $i ." ', 'MODULE_SHIPPING_ZONES_HANDLING_" . $i."', '0', 'Накладные расходы для зоны доставки', '6', '0', now())");
